...
def preprocess_Rt():
    ...
    final_rt.to_csv("data/rt_data.csv", index=False, sep=',')
...
def align_rt(update=True, train=True):
    ...
    county_rt = pd.read_csv("data/rt_data.csv", dtype={"FIPS":str})

    case_data = pd.read_csv("data/jhu_data.csv", dtype={"FIPS":str})
    final = pd.merge(left=county_rt, right=case_data,how="left", on=['FIPS', 'date'], copy=False)

    testing_data = pd.read_csv("data/testing_data.csv", dtype={"FIPS":str})
    ...
    if update:
        ...
        new_col = final_estimate.groupby("FIPS", as_index=False).apply(lambda x : get_optimal_lag(x['test_positivity_norm'], x['rt_mean_MIT'], 0)).reset_index(drop=True)
        ...
        pickle.dump(without_county_rt_, open("data/without_county_rt.p", "wb"))
        pickle.dump(with_county_rt_, open("data/with_county_rt.p", "wb"))
    ...
    if train:
        ...
        new_col = final_estimate.groupby("FIPS", as_index=False).apply(lambda x : get_optimal_lag(x['test_positivity_norm'], x['rt_mean_MIT'], predict)).reset_index(drop=True)
        final_estimate[["aligned_state_rt","ECR_shift"]] = new_col
        ...
        without_county_rt_dict = pickle.load(open("data/without_county_rt.p", "rb"))
        with_county_rt_dict = pickle.load(open("data/with_county_rt.p", "rb"))
        ...
        combined.to_csv("data/processed_rt.csv", index=False)
        return combined

        #combined.plot.scatter(x="normalized_cases_norm", y='prediction')
        #plt.show()
        #final.plot.scatter(x="normalized_cases_norm", y='RtIndicator')
        #plt.show()
        #final_estimate['cases_shift'] = final_estimate.groupby("FIPS", as_index=False)['normalized_cases_norm'].shift(periods=-10)
        #final_estimate = final_estimate.dropna()
        #print(final_estimate['prediction'].corr(final_estimate['normalized_cases_norm']))
        #print(r2_score(final_estimate['normalized_cases_norm'],final_estimate['prediction']))
        #print(final_estimate['prediction'].corr(final_estimate['cases_shift']))
        #print(r2_score(final_estimate['cases_shift'],final_estimate['prediction']))
    ...
    # 3. Normalize
    # 4. Compute state Rt
    # 5. Split and compute county Rt in segment with this data
    # 6. For each dataframe, use dictionary to shift so that Rt value today corresponds to cases for when you want to predict
    # 7. Run regression model to predict cases
    # 8. Save prediction in column
    # 9. Add column to each dataframe and then merge
...
def merge_data(save_files = False, ag=False):
    ...
    print("[ ] Align Rt values", end='\r')
    processed_rt = align_rt()
    print('[' + u'\u2713' + ']\n')
    ...
    processed_rt['FIPS'] = processed_rt['FIPS'].astype(int)
    processed_rt['date'] = processed_rt['date'].astype(str)

    merged_DF = pd.merge(left=processed_rt, right=testing, how='left', on=['FIPS', 'date'], copy=False)
