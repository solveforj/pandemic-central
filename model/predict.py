...
def predict():
    ...
    past = projections.groupby("FIPS").head(4).reset_index(drop=True)
    ...
    projections['shift'] = [7, 14, 21, 28] * int(len(projections)/4)
    ...
    projections['cases'] = (projections['combined_predictions'] * projections['TOT_POP'] / 100000)
    ...
    past['cases'] = (past['confirmed_cases_norm'] * past['TOT_POP'] / 100000)
    ...
    combined_web = pd.concat([past, projections], axis=0).sort_values(['FIPS','date']).reset_index(drop=True).fillna("NULL")
