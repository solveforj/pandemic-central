...
#clf = RandomForestClassifier(max_depth=50, random_state=0).fit(X_train.iloc[:,2:], y_train)
#regr = MLPRegressor(random_state=1, max_iter=1000).fit(X_train.iloc[:,2:], y_train)
#regr = SVR(verbose=True, n_jobs=4).fit(X_train.iloc[:,2:], y_train)
...
data1.to_csv("predictions.csv", index=None, sep=",")
#data2 = data[data['POP_DENSITY'] < 5000]
#X = data2[data.columns[:-1]]
#y = data2['label']
#print(regr.score(X.iloc[:,2:], y))
